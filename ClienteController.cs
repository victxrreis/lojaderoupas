...
public class ClienteController : ControllerBase
{
   ...
   public async Task<ActionResult<Cliente>> Alterar(Cliente cliente)
   {
      ...
      var clienteTemp = await _context.Cliente.FindAsync(cliente.ClienteNome);
      if (clienteTemp is null) return NotFound();
      _context.Cliente.Update(clienteTemp);
      await _context.SaveChangesAsync();
      return Ok();
   }
   ...
   public async Task<ActionResult> MudarEndereco(int id, [FromForm] string clienteEndereco)
   {
      if (_context is null) return NotFound();
      if (_context.Cliente is null) return NotFound();
      var clienteTemp = await _context.Cliente.FindAsync(clienteEndereco);
      if (clienteTemp is null) return NotFound();
   ...
   [HttpPatch()]
   [Route("mudar/{id}")]
   public async Task<ActionResult> MudarEmail(int id, [FromForm] string clienteEmail)
   {
      if (_context is null) return NotFound();
      if (_context.Cliente is null) return NotFound();
